...
function damm_checksum_8digit(value) {
    if (value >= 100000000)
        throw new Error(
            'Expected value less than 100000000, instead got ' + value
        );
...
Types.asset = {
    fromByteBuffer: function fromByteBuffer(b) {
        ...
        if (precision >= 16) {
            ...
            symbol =
                '@@' +
                nai.toString().padStart(8, '0') +
                damm_checksum_8digit(nai).to_String();
            precision = precision % 16;
            b.skip(3);
            amount_string = (0, _number_utils.fromImpliedDecimal)(
                amount,
                precision
            );
        } else {
            ...
            symbol = new Buffer(_b_copy.toBinary(), 'binary')
                .toString()
                .replace(/\x00/g, '');
            b.skip(7);
        ...
        if (object['nai']) {
            ...
            switch (object['nai']) {
                case '@@000000021':
                    precision = 3;
                    symbol =
                        _config2.default.get('address_prefix') == 'STM'
                            ? 'STEEM'
                            : 'TESTS';
                    break;
                case '@@000000013':
            ...
            if (symbol.startsWith('@@')) {
                ...
                throw new Error(
                    'Symbols are not longer than 6 characters ' +
                        symbol +
                        '-' +
                        symbol.length
                );
...
Types.asset_symbol = {
    fromByteBuffer: function fromByteBuffer(b) {
        ...
        if (precision >= 16) {
            ...
            var symbol = new Buffer(_b_copy2.toBinary(), 'binary')
                .toString()
                .replace(/\x00/g, '');
            if (symbol == 'STEEM' || symbol == 'TESTS')
                ...
                throw new Error(
                    "Expecting non-smt core asset symbol, instead got '" +
                        symbol +
                        "'"
                );
            b.skip(7);
    ...
    appendByteBuffer: function appendByteBuffer(b, object) {
        ...
        if (!object['nai'].startsWith('@@'))
            throw new Error(
                "Asset Symbols NAIs must be prefixed with '@@'. Was " +
                    object['nai']
            );
    ...
    return {
        ...
        opTypeId: function opTypeId(value) {
            ...
            if (typeof value === 'number') type_id = value;
                ...
                try {
                    for (
                        var _iterator = this.st_operations[Symbol.iterator](),
                            _step;
...
Types.public_key = {
    ...
    toObject: function toObject(object) {
        ...
        if (debug.use_default && object === undefined) {
            return (
                _ecc.ecc_config.get('address_prefix') +
