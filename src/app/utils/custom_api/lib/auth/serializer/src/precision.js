...
module.exports._internal = _internal =
    // Warning: Long operations may over-flow without detection
    {
        ...
        decimal_precision_string: function decimal_precision_string(
            number,
            precision
        ) {
            var error_info =
            ...
            if (!number_parts) {
                throw new Error(
                    'Invalid number: ' + number_string + ' ' + error_info
                );
            }
            ...
            while (/0$/.test(decimal_part)) {
                decimal_part = decimal_part.substring(
                    0,
